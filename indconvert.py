...
def indentation_converter(line, ind_char=' ', old_ind=2, new_ind=4):
    ...
    if indcount % old_ind > 0:
        raise IndentationException(
            'The number of indentation not divisible by indentation count: ' + str(old_ind))
...
def parse_args(args):
    ...
    parser.add_argument('-f', '--filename', type=str,
                        help='The filename of the file that needs its indentation changed', required=True)
    parser.add_argument('-i', '--indchar', type=str,
                        help='The character used for indentation', required=True)
    parser.add_argument('-o', '--oldind', type=int,
                        help='The existing number of indentation characters used (for one indentation)', required=True)
    parser.add_argument('-n', '--newind', type=int,
                        help='The new wanted number of indentation characters', required=True)
    ...
    with open(filename, 'r') as infile:
        for line in infile:
            with open(filename + '_out', 'a') as outfile:
                ...
                outfile.write(indentation_converter(
                    line, indchar, oldind, newind))
